...
#%%   grilla de alfas
delta=1/(a*3)
xfinn=1.5*k_0*(epsilon_2*mu_2)**0.5  # en general no hace falta ir hasta 2*k_2. Con considerar hasta 1.5k_2 alcanza 
...
indice_a0=np.argwhere(alfas==alfa_0)
start_time = time.time()
bt=np.zeros((len(alfas)),dtype=complex)
bt[np.r_[0:indice_a0, (indice_a0+1):len(alfas)]] = np.array(list(map(b_T,alfas[np.r_[0:indice_a0, (indice_a0+1):len(alfas)]])))
bt[indice_a0]=T*(sigma_2*beta_alfa(alfa_0,epsilon_1,mu_1)/sigma_1+beta_alfa(alfa_0,epsilon_2,mu_2))*(2*np.pi*D(0,beta_alfa(alfa_0,epsilon_2,mu_2)-beta_alfa(alfa_0,epsilon_1,mu_1))-a)

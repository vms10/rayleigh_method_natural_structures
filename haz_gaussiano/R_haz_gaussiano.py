...
#%%  solucion particular de Ax=b. Esto cambia respecto onda plana incidente
start_time = time.time()
br=np.zeros((len(alfas_R)),dtype=complex)
br[:] = np.array(list(map(b_R,alfas_R)))
...
R = lambda alpha: (sigma_2*beta_alfa(alpha,epsilon_1,mu_1)/sigma_1-beta_alfa(alpha,epsilon_2,mu_2))/(sigma_2*beta_alfa(alpha,epsilon_1,mu_1)/sigma_1+beta_alfa(alpha,epsilon_2,mu_2))

resta=[]
for i in range(len(x_R)):
    resta.append(np.absolute(x_R[i]-R(alfas_R[i])*A_alfa(alfas_R[i]))**2)
resta=np.array(resta)

from itertools import repeat
betas_R=np.array(list(map(beta_alfa,alfas_R,repeat(epsilon_1),repeat(mu_1))))
put=resta*betas_R
put=put/beta_alfa(alfa_0,epsilon_1,mu_1)
#%% curva de intensidad rugosidades vs alfa/k_1
# Esta curva es la que mas sentido tiene 
from matplotlib.ticker import (MultipleLocator, FormatStrFormatter,
                               AutoMinorLocator)
fig, ax = plt.subplots(figsize=(12.8,9.6))

ax.plot(alfa_k1,put,'bo',markersize=3)
...
plt.ylabel('Intensidad rugosidades', fontsize=32, labelpad=20)
